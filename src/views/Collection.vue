	...
	<section>
		<v-toolbar flat>
			<div class="collection-nav-bar">
				<span class="material-icons" @click="popRoute()">
					arrow_back </span
				><span
					class="material-icons"
					@click="isShowCollectionOptions = true"
				>
					more_vert
				</span>
			</div>
				...
				<v-list-item v-for="(verse, key) in verses" :key="key">
					<v-list-item-content>
						...
						<v-list-item-title
							...
							"
							>{{ verse.book.name }}
							{{ verse.chapter }}</v-list-item-title
						>
		...
		<v-dialog v-model="isShowDeleteConfirmationDialog">
			<v-card>
				...
				<v-card-text
					>Deleted collection cannot be restored.</v-card-text
				>
				<v-card-actions>
					<v-spacer></v-spacer>
					<v-btn text @click="isShowDeleteConfirmationDialog = false"
						>Cancel</v-btn
					>
					<v-btn depressed rounded @click="removeCollection()"
						>Confirm</v-btn
					>
				</v-card-actions>
			</v-card>
		...
		methods: {
				...
				db.collection("users")
					...
					.then((querySnapshot) => {
						if (querySnapshot.data().lastReview != "") {
							let date = new Date(
								querySnapshot.data().lastReview.seconds * 1000
							);
					...
					.catch((error) => {
						console.log(
							"Error getting users collection info",
							error
						);
					});
			},
			...
			removeVerseFromCollection() {
				...
				db.collection("users")
					...
					.then(() => {
						console.log(
							"Successfully remove verse from the collection"
						);
					})
					.catch((error) => {
						console.log(
							"Error removing verse from the collection",
							error
						);
					});
			...
			changeReviewPeriod(period) {
				...
				db.collection("users")
					...
					.then(() => {
						console.log(
							"Successfully the collection's review period"
						);
					})
					.catch((error) => {
						console.log(
							"Error to update the collection's review period",
							error
						);
					});
